...
def create_app():
    ...
    def login():
        if request.method == "POST":
            ...
            password_ok = bool(user and password_matches(password, user.password_hash))
            if password_ok:
                login_user(user)
    ...
    def profile():
        if request.method == "POST":
            ...
            if existing_goal:
                ...
                db.session.add(BodyGoal(user_id=current_user.id, goal=recommended_goal, reason=reason))
            ...
            try:
                ...
                flash("Unable to save your profile.", "error")
    ...
    def workout():
        ...
        plan = workout_for_goal(goal, profile.experience if profile else "beginner", profile.bmi if profile else 22)
        existing_workout = Workout.query.filter_by(user_id=current_user.id).first()

        if existing_workout:
            existing_workout.goal = goal
            existing_workout.day_plan = json.dumps(plan)
        else:
            db.session.add(
                Workout(
                    user_id=current_user.id,
                    goal=goal,
                    day_plan=json.dumps(plan)
                )
            )
    ...
    def yoga():
        ...
        if request.method == "POST":
            db.session.add(Yoga(user_id=current_user.id, focus=focus, poses=poses))
            db.session.commit()
    ...
    def ai_coach():
        ...
        try:
            ...
            ai_data = {
                "error": "Unable to generate your fitness plan at the moment. Please try again later."
            }
